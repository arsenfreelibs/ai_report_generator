...
class ReportExampleIndexer:
    ...
    def _parse_report_file(self, file_path: str) -> Dict:
        """Parse a report example file and extract sections"""
        try:
    ...
    def extract_index_data(self) -> List[Dict]:
        """Extract report examples from knowledge directory for indexing"""
        index_data = []
        ...
        # Process all files in the report_examples directory
        for filename in os.listdir(self.knowledge_path):
            ...
            sections = self._parse_report_file(file_path)
            
            if not sections:
                continue

            # Create indexable content combining all sections
            content_parts = []
        ...
        logger.info(f"Extracted {len(index_data)} report examples for indexing")
        return index_data

    def create_indexes(self):
        """Create vector store and BM25 index for report examples"""
        if not self.index_data:
            self.extract_index_data()

        if not self.index_data:
            logger.warning("No report examples found to index")
            return None

        # Create embeddings using sentence-transformers
        embedding_model = SentenceTransformerEmbeddings(model_name=self.embedding_model_name)

        # Create FAISS index for report examples
        ...
        logger.info(f"Created report example indexes with {self.vector_store.index.ntotal} examples")
        return self.vector_store

    def search(self, query: str, k: int = 5) -> List[Dict]:
        """Search report examples using hybrid retrieval"""
